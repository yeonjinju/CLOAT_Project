...
<body>
	<section class="content board view list">
		...
		<div class="inner">
				...
				<div class="view_top">
					...
					<div class="view_info">
						<div class="view_info_lt">
							<p>
								<span>작성자</span> ${qna.id}
							</p>
							<p>
								<span>조회수</span> ${qna.qna_views}
							</p>
						</div>
						<div class="view_info_rt">
							<p>
								<span>작성날짜</span>
								<fmt:formatDate value="${qna.created_at}"
									pattern="yyyy.MM.dd HH:mm:ss" />
							</p>
						</div>
					</div>
				</div>
				<div class="view_btm"> ${qna.qna_content}
					<c:if test="${not empty qna.qna_file}">
						<div class="view_file">
							<a
								href="${pageContext.request.contextPath}/upload/${qna.qna_file}"
								download>${qna.qna_file}</a>
						</div>
					</c:if>
				</div>
			</div>
								...
								<div class="cmt_box">
									...
									<div class="cmt_btm">
										<div class="answerDiv">
											...
											<c:if test="${sessionScope.mvo.user_type eq 'ADMIN'}">
												<div class="cmt_bttn bttn_wrap">
													...
													<form
														action="${pageContext.request.contextPath}/deleteQnaAnswer?id=${mvo.id}"
														method="post">
														<input type="hidden" name="answer_idx"
															value="${answer.answer_idx}" /> <input type="hidden"
										...
										<div class="answerTextarea">
											<form
												action="${pageContext.request.contextPath}/updateQnaAnswer?id=${mvo.id}"
												method="post">
							...
							<c:if test="${sessionScope.mvo.user_type eq 'ADMIN'}">
								<li>
									<div class="cmt_box">
										...
										<div class="cmt_btm">
											<form
												action="${pageContext.request.contextPath}/writeQnaAnswer?id=${mvo.id}"
												method="post">
												<textarea name="answer_content" class="txtarea"
			...
			<div class="bttn_wrap">
				<div>
					<a href="QnaWrite" class="bttn ipt_sbm">글쓰기</a>
					<!-- 해당 글 작성자 또는 관리자일 경우 버튼 표시 -->
					<c:if
						test="${sessionScope.mvo.id eq qna.id or sessionScope.mvo.user_type eq 'ADMIN'}">
						<a
							href="${pageContext.request.contextPath}/qnaEdit?no=${qna.qna_idx}">수정</a>

						<button class="bttn" onclick="confirmDelete(${qna.qna_idx})">삭제</button>
					</c:if>
				</div>
				<div class="ml-auto">

					<a
						href="${pageContext.request.contextPath}/QnaList?pageNum=${pageNum}"
	...
	<script>
		function confirmDelete(qna_idx) {
		    if (confirm('해당 게시글을 삭제하시겠습니까?')) {
		        ...
		        location.href = '${pageContext.request.contextPath}/qnaDelete?no=' + qna_idx;
		    }
		}
	</script>
