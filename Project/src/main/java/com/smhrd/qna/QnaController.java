...
public class QnaController {
	...
	public String qnaView(@RequestParam("no") int qnaIdx,
		...
		// "null" 문자열을 실제 null로 변환 (첨부파일 필드에 대해)
		if (qna != null && "null".equals(qna.getQna_file())) {
	...
	public String updateQnaEdit(QnaVO vo, HttpSession session,
		...
		System.out.println("updateQnaEdit() 호출됨 - 수정 처리 시작");
		System.out.println("수정 대상 글 번호: " + vo.getQna_idx());
		System.out.println("기존 파일명: " + oldFile);
		System.out.println("첨부된 새 파일: " + (file != null ? file.getOriginalFilename() : "없음"));

		// 세션에서 로그인 ID 가져오기
		String sessionId = (String) session.getAttribute("id");
		System.out.println("세션에서 가져온 로그인 ID: " + sessionId);

		vo.setAdmin_id(sessionId);
		...
		if (file != null && !file.isEmpty()) {
			fileDemo = file.getOriginalFilename();
			System.out.println("새 파일명: " + fileDemo);
			try {
				// UUID 붙여서 파일명 변경 (중복 방지)
				String baseName = fileDemo.substring(0, fileDemo.lastIndexOf("."));
				String extension = fileDemo.substring(fileDemo.lastIndexOf("."));
				fileDemo = baseName + "_" + UUID.randomUUID().toString() + extension;
				System.out.println("변경된 저장 파일명: " + fileDemo);
				...
				fos.write(file.getBytes());
				fos.close();
				System.out.println("새 파일 저장 완료: " + targetFile.getAbsolutePath());
		...
		// VO에 최종 파일명 세팅
		vo.setQna_file(fileDemo);
		System.out.println("vo.setQna_file 호출됨, 파일명: " + fileDemo);
